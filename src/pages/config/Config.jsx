...
export default function App() {
  ...
  // const paginatedData = (itemData || []).slice(startIndex, endIndex);
  const paginatedData = Array.isArray(itemData)
  ? itemData.slice(startIndex, endIndex)
  : [];
...
setItemData(dataArray);
      setTotalItems(res?.totalItems || dataArray.length);
    } catch (err) {
      console.error(err);
      setItemData([]);
    } finally {
...
{activeTab === "tax" && (
  <div className="responsive-flex-container">
    ...
    >
      ...
      <div className="fw-semibold small me-3 mb-0 text-wrap">
        {isTaxEnabled
          ? "Tax configuration is enabled"
          : "Tax configuration is disabled"}
      </div>
