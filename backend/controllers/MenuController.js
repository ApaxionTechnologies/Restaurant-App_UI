...
export const addMenuItem = async (req, res) => {
  try {
    ...
    const status = ["Published", "Draft"].includes(rawStatus) ? rawStatus : "Published";
    // ==========================
// In addMenuItem function, update the menu item creation:
restaurant.menu.push({
  name,
  price,
  category,
  ...
  status,
  type: type || "veg",
  discount: req.body.discount ? parseFloat(req.body.discount) : 0 // Add this line
});
    await restaurant.save();
...
//   try {
//     const restaurant = req.restaurant; // set by requireAuth
//     if (!restaurant) return res.status(401).json({ message: "Unauthorized" });
...
//     res.status(200).json({ message: "Status updated", menuItem });
//   } catch (err) {
//     console.error(err);
